...
namespace Adyen.DisputeWebhooks.Models
{
    ...
    /// </summary>
    [JsonConverter(typeof(DisputeEventNotificationJsonConverter))]
    ...
    public class DisputeEventNotificationJsonConverter : JsonConverter<DisputeEventNotification>
    {
        ...
        public override DisputeEventNotification Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (balancePlatform.IsSet)
                disputeEventNotification.BalancePlatform = balancePlatform.Value;
            if (creationDate.IsSet)
                disputeEventNotification.CreationDate = creationDate.Value;
            if (description.IsSet)
                disputeEventNotification.Description = description.Value;
            if (disputedAmount.IsSet)
                disputeEventNotification.DisputedAmount = disputedAmount.Value;
            if (id.IsSet)
                disputeEventNotification.Id = id.Value;
            if (status.IsSet)
                disputeEventNotification.Status = status.Value;
            if (statusDetail.IsSet)
                disputeEventNotification.StatusDetail = statusDetail.Value;
            if (transactionId.IsSet)
                disputeEventNotification.TransactionId = transactionId.Value;
            if (type.IsSet)
                disputeEventNotification.Type = type.Value;
            return disputeEventNotification;
        }
