...
namespace Adyen.Payout.Models
{
    ...
    /// PayoutResponse.
    /// </summary>
    [JsonConverter(typeof(PayoutResponseJsonConverter))]
    ...
    public class PayoutResponseJsonConverter : JsonConverter<PayoutResponse>
    {
        ...
        public override PayoutResponse Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (additionalData.IsSet)
                payoutResponse.AdditionalData = additionalData.Value;
            if (authCode.IsSet)
                payoutResponse.AuthCode = authCode.Value;
            if (dccAmount.IsSet)
                payoutResponse.DccAmount = dccAmount.Value;
            if (dccSignature.IsSet)
                payoutResponse.DccSignature = dccSignature.Value;
            if (fraudResult.IsSet)
                payoutResponse.FraudResult = fraudResult.Value;
            if (issuerUrl.IsSet)
                payoutResponse.IssuerUrl = issuerUrl.Value;
            if (md.IsSet)
                payoutResponse.Md = md.Value;
            if (paRequest.IsSet)
                payoutResponse.PaRequest = paRequest.Value;
            if (pspReference.IsSet)
                payoutResponse.PspReference = pspReference.Value;
            if (refusalReason.IsSet)
                payoutResponse.RefusalReason = refusalReason.Value;
            if (resultCode.IsSet)
                payoutResponse.ResultCode = resultCode.Value;
