...
namespace Adyen.Payout.Models
{
    ...
    /// PayoutRequest.
    /// </summary>
    [JsonConverter(typeof(PayoutRequestJsonConverter))]
    ...
    public class PayoutRequestJsonConverter : JsonConverter<PayoutRequest>
    {
        ...
        public override PayoutRequest Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (card.IsSet)
                payoutRequest.Card = card.Value;
            if (fraudOffset.IsSet)
                payoutRequest.FraudOffset = fraudOffset.Value;
            if (fundSource.IsSet)
                payoutRequest.FundSource = fundSource.Value;
            if (recurring.IsSet)
                payoutRequest.Recurring = recurring.Value;
            if (selectedRecurringDetailReference.IsSet)
                payoutRequest.SelectedRecurringDetailReference = selectedRecurringDetailReference.Value;
            if (shopperEmail.IsSet)
                payoutRequest.ShopperEmail = shopperEmail.Value;
            if (shopperInteraction.IsSet)
                payoutRequest.ShopperInteraction = shopperInteraction.Value;
            if (shopperName.IsSet)
                payoutRequest.ShopperName = shopperName.Value;
            if (shopperReference.IsSet)
                payoutRequest.ShopperReference = shopperReference.Value;
            if (telephoneNumber.IsSet)
                payoutRequest.TelephoneNumber = telephoneNumber.Value;
        ...
        public void WriteProperties(Utf8JsonWriter writer, PayoutRequest payoutRequest, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (payoutRequest._FraudOffsetOption.IsSet)
                if (payoutRequest._FraudOffsetOption.Value != null)
