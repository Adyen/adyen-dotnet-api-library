...
namespace Adyen.Payout.Models
{
    ...
    /// </summary>
    [JsonConverter(typeof(StoreDetailAndSubmitRequestJsonConverter))]
    ...
    public class StoreDetailAndSubmitRequestJsonConverter : JsonConverter<StoreDetailAndSubmitRequest>
    {
        ...
        public override StoreDetailAndSubmitRequest Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (reference.IsSet)
                storeDetailAndSubmitRequest.Reference = reference.Value!;
            if (shopperEmail.IsSet)
                storeDetailAndSubmitRequest.ShopperEmail = shopperEmail.Value!;
            if (shopperReference.IsSet)
                storeDetailAndSubmitRequest.ShopperReference = shopperReference.Value!;
            if (additionalData.IsSet)
                storeDetailAndSubmitRequest.AdditionalData = additionalData.Value;
            if (bank.IsSet)
                storeDetailAndSubmitRequest.Bank = bank.Value;
            if (billingAddress.IsSet)
                storeDetailAndSubmitRequest.BillingAddress = billingAddress.Value;
            if (card.IsSet)
                storeDetailAndSubmitRequest.Card = card.Value;
            if (fraudOffset.IsSet)
                storeDetailAndSubmitRequest.FraudOffset = fraudOffset.Value;
            if (selectedBrand.IsSet)
                storeDetailAndSubmitRequest.SelectedBrand = selectedBrand.Value;
            if (shopperName.IsSet)
                storeDetailAndSubmitRequest.ShopperName = shopperName.Value;
            if (shopperStatement.IsSet)
                storeDetailAndSubmitRequest.ShopperStatement = shopperStatement.Value;
            if (socialSecurityNumber.IsSet)
                storeDetailAndSubmitRequest.SocialSecurityNumber = socialSecurityNumber.Value;
            if (telephoneNumber.IsSet)
                storeDetailAndSubmitRequest.TelephoneNumber = telephoneNumber.Value;
