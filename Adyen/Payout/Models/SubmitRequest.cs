...
namespace Adyen.Payout.Models
{
    ...
    /// SubmitRequest.
    /// </summary>
    [JsonConverter(typeof(SubmitRequestJsonConverter))]
    ...
    public class SubmitRequestJsonConverter : JsonConverter<SubmitRequest>
    {
        ...
        public override SubmitRequest Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (dateOfBirth.IsSet)
                submitRequest.DateOfBirth = dateOfBirth.Value;
            if (entityType.IsSet)
                submitRequest.EntityType = entityType.Value;
            if (fraudOffset.IsSet)
                submitRequest.FraudOffset = fraudOffset.Value;
            if (nationality.IsSet)
                submitRequest.Nationality = nationality.Value;
            if (shopperName.IsSet)
                submitRequest.ShopperName = shopperName.Value;
            if (shopperStatement.IsSet)
                submitRequest.ShopperStatement = shopperStatement.Value;
            if (socialSecurityNumber.IsSet)
                submitRequest.SocialSecurityNumber = socialSecurityNumber.Value;
