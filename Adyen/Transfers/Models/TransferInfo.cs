...
namespace Adyen.Transfers.Models
{
    ...
    /// TransferInfo.
    /// </summary>
    [JsonConverter(typeof(TransferInfoJsonConverter))]
    public partial class TransferInfo
    {
        ...
        public class PrioritiesEnum : IEnum
        {
            ...
            /// <summary>
            /// Compares two <see cref="PrioritiesEnum"/> instances for equality.
            /// </summary>
            ...
            /// <summary>
            /// Compares two <see cref="PrioritiesEnum"/> instances for inequality.
            /// </summary>
            public static bool operator !=(PrioritiesEnum? left, PrioritiesEnum? right) => !string.Equals(left?.Value, right?.Value, StringComparison.OrdinalIgnoreCase);

            /// <summary>
            /// Returns true if the given object is equal to this <see cref="PrioritiesEnum"/> instance.
            /// </summary>
            ...
            /// <summary>
            /// Returns a hash code for this <see cref="PrioritiesEnum"/> instance.
            /// </summary>
            public override int GetHashCode() => Value?.GetHashCode() ?? 0;

            /// <summary>
            /// Returns the string value of the <see cref="PrioritiesEnum"/> instance.
            /// </summary>
            public override string ToString() => Value ?? string.Empty;
            ...
            {
                /// <summary>
                /// Deserializes a <see cref="PrioritiesEnum"/> from JSON.
                /// </summary>
                ...
                /// <summary>
                /// Serializes a <see cref="PrioritiesEnum"/> to JSON.
                /// </summary>
    ...
    public class TransferInfoJsonConverter : JsonConverter<TransferInfo>
    {
        ...
        public override TransferInfo Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (description.IsSet)
                transferInfo.Description = description.Value;
            if (executionDate.IsSet)
                transferInfo.ExecutionDate = executionDate.Value;
            if (paymentInstrumentId.IsSet)
                transferInfo.PaymentInstrumentId = paymentInstrumentId.Value;
            if (priorities.IsSet)
                transferInfo.Priorities = priorities.Value;
            if (priority.IsSet)
                transferInfo.Priority = priority.Value;
            if (reference.IsSet)
                transferInfo.Reference = reference.Value;
            if (referenceForBeneficiary.IsSet)
                transferInfo.ReferenceForBeneficiary = referenceForBeneficiary.Value;
            if (review.IsSet)
                transferInfo.Review = review.Value;
            if (type.IsSet)
                transferInfo.Type = type.Value;
            if (ultimateParty.IsSet)
                transferInfo.UltimateParty = ultimateParty.Value;
            return transferInfo;
        }
