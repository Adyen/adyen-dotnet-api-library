...
namespace Adyen.Transfers.Models
{
    ...
    /// PartyIdentification.
    /// </summary>
    [JsonConverter(typeof(PartyIdentificationJsonConverter))]
    ...
    public class PartyIdentificationJsonConverter : JsonConverter<PartyIdentification>
    {
        ...
        public override PartyIdentification Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (dateOfBirth.IsSet)
                partyIdentification.DateOfBirth = dateOfBirth.Value;
            if (email.IsSet)
                partyIdentification.Email = email.Value;
            if (firstName.IsSet)
                partyIdentification.FirstName = firstName.Value;
            if (fullName.IsSet)
                partyIdentification.FullName = fullName.Value;
            if (lastName.IsSet)
                partyIdentification.LastName = lastName.Value;
            if (reference.IsSet)
                partyIdentification.Reference = reference.Value;
            if (type.IsSet)
                partyIdentification.Type = type.Value;
            if (url.IsSet)
                partyIdentification.Url = url.Value;
