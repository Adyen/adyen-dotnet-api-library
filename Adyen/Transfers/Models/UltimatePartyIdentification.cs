...
namespace Adyen.Transfers.Models
{
    ...
    /// </summary>
    [JsonConverter(typeof(UltimatePartyIdentificationJsonConverter))]
    ...
    public class UltimatePartyIdentificationJsonConverter : JsonConverter<UltimatePartyIdentification>
    {
        ...
        public override UltimatePartyIdentification Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
                ...
                if (utf8JsonReader.TokenType == JsonTokenType.PropertyName && currentDepth == utf8JsonReader.CurrentDepth - 1)
                {
                    ...
                    switch (jsonPropertyName)
                    {
                        ...
                        case "type":
                            ...
                            type = new Option<UltimatePartyIdentification.TypeEnum?>(UltimatePartyIdentification.TypeEnum.FromStringOrDefault(typeRawValue) ?? (UltimatePartyIdentification.TypeEnum)typeRawValue);
                            break;
                        case "url":
            ...
            var ultimatePartyIdentification = new UltimatePartyIdentification();
            if (address.IsSet)
                ultimatePartyIdentification.Address = address.Value;
            if (dateOfBirth.IsSet)
                ultimatePartyIdentification.DateOfBirth = dateOfBirth.Value;
            if (email.IsSet)
                ultimatePartyIdentification.Email = email.Value;
            if (firstName.IsSet)
                ultimatePartyIdentification.FirstName = firstName.Value;
            if (fullName.IsSet)
                ultimatePartyIdentification.FullName = fullName.Value;
            if (fundingInstrument.IsSet)
                ultimatePartyIdentification.FundingInstrument = fundingInstrument.Value;
            if (lastName.IsSet)
                ultimatePartyIdentification.LastName = lastName.Value;
            if (reference.IsSet)
                ultimatePartyIdentification.Reference = reference.Value;
            if (type.IsSet)
                ultimatePartyIdentification.Type = type.Value;
            if (url.IsSet)
                ultimatePartyIdentification.Url = url.Value;
            return ultimatePartyIdentification;
        }
