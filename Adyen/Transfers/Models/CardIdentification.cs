...
namespace Adyen.Transfers.Models
{
    ...
    /// CardIdentification.
    /// </summary>
    [JsonConverter(typeof(CardIdentificationJsonConverter))]
    ...
    public class CardIdentificationJsonConverter : JsonConverter<CardIdentification>
    {
        ...
        public override CardIdentification Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (expiryYear.IsSet)
                cardIdentification.ExpiryYear = expiryYear.Value;
            if (issueNumber.IsSet)
                cardIdentification.IssueNumber = issueNumber.Value;
            if (number.IsSet)
                cardIdentification.Number = number.Value;
            if (startMonth.IsSet)
                cardIdentification.StartMonth = startMonth.Value;
            if (startYear.IsSet)
                cardIdentification.StartYear = startYear.Value;
            if (storedPaymentMethodId.IsSet)
                cardIdentification.StoredPaymentMethodId = storedPaymentMethodId.Value;
