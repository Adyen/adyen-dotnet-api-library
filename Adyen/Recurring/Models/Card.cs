    ...
    public class CardJsonConverter : JsonConverter<Card>
    {
        ...
        public override Card Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (expiryMonth.IsSet)
                card.ExpiryMonth = expiryMonth.Value;
            if (expiryYear.IsSet)
                card.ExpiryYear = expiryYear.Value;
            if (holderName.IsSet)
                card.HolderName = holderName.Value;
            if (issueNumber.IsSet)
                card.IssueNumber = issueNumber.Value;
            if (number.IsSet)
                card.Number = number.Value;
            if (startMonth.IsSet)
                card.StartMonth = startMonth.Value;
            if (startYear.IsSet)
                card.StartYear = startYear.Value;
            return card;
        }
