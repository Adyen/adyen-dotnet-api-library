...
namespace Adyen.Recurring.Models
{
    ...
    public class BankAccountJsonConverter : JsonConverter<BankAccount>
    {
        ...
        public override BankAccount Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (bankCity.IsSet)
                bankAccount.BankCity = bankCity.Value;
            if (bankLocationId.IsSet)
                bankAccount.BankLocationId = bankLocationId.Value;
            if (bankName.IsSet)
                bankAccount.BankName = bankName.Value;
            if (bic.IsSet)
                bankAccount.Bic = bic.Value;
            if (countryCode.IsSet)
                bankAccount.CountryCode = countryCode.Value;
            if (iban.IsSet)
                bankAccount.Iban = iban.Value;
            if (ownerName.IsSet)
                bankAccount.OwnerName = ownerName.Value;
            if (taxId.IsSet)
                bankAccount.TaxId = taxId.Value;
            return bankAccount;
        }
