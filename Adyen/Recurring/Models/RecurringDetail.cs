...
namespace Adyen.Recurring.Models
{
    ...
    /// RecurringDetail.
    /// </summary>
    [JsonConverter(typeof(RecurringDetailJsonConverter))]
    ...
    public class RecurringDetailJsonConverter : JsonConverter<RecurringDetail>
    {
        ...
        public override RecurringDetail Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (alias.IsSet)
                recurringDetail.Alias = alias.Value;
            if (aliasType.IsSet)
                recurringDetail.AliasType = aliasType.Value;
            if (bank.IsSet)
                recurringDetail.Bank = bank.Value;
            if (billingAddress.IsSet)
                recurringDetail.BillingAddress = billingAddress.Value;
            if (card.IsSet)
                recurringDetail.Card = card.Value;
            if (contractTypes.IsSet)
                recurringDetail.ContractTypes = contractTypes.Value;
            if (creationDate.IsSet)
                recurringDetail.CreationDate = creationDate.Value;
            if (firstPspReference.IsSet)
                recurringDetail.FirstPspReference = firstPspReference.Value;
            if (name.IsSet)
                recurringDetail.Name = name.Value;
            if (networkTxReference.IsSet)
                recurringDetail.NetworkTxReference = networkTxReference.Value;
            if (paymentMethodVariant.IsSet)
                recurringDetail.PaymentMethodVariant = paymentMethodVariant.Value;
            if (shopperName.IsSet)
                recurringDetail.ShopperName = shopperName.Value;
