...
namespace Adyen.Management.Models
{
    ...
    /// Store.
    /// </summary>
    [JsonConverter(typeof(StoreJsonConverter))]
    ...
    public class StoreJsonConverter : JsonConverter<Store>
    {
        ...
        public override Store Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (businessLineIds.IsSet)
                store.BusinessLineIds = businessLineIds.Value;
            if (description.IsSet)
                store.Description = description.Value;
            if (externalReferenceId.IsSet)
                store.ExternalReferenceId = externalReferenceId.Value;
            if (id.IsSet)
                store.Id = id.Value;
            if (localizedInformation.IsSet)
                store.LocalizedInformation = localizedInformation.Value;
            if (merchantId.IsSet)
                store.MerchantId = merchantId.Value;
            if (phoneNumber.IsSet)
                store.PhoneNumber = phoneNumber.Value;
            if (reference.IsSet)
                store.Reference = reference.Value;
            if (shopperStatement.IsSet)
                store.ShopperStatement = shopperStatement.Value;
            if (splitConfiguration.IsSet)
                store.SplitConfiguration = splitConfiguration.Value;
            if (status.IsSet)
                store.Status = status.Value;
            if (subMerchantData.IsSet)
                store.SubMerchantData = subMerchantData.Value;
            return store;
        }
        ...
        public void WriteProperties(Utf8JsonWriter writer, Store store, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (store._LocalizedInformationOption.IsSet)
            {
                writer.WritePropertyName("localizedInformation");
                JsonSerializer.Serialize(writer, store.LocalizedInformation, jsonSerializerOptions);
            }
