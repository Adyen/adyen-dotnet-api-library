...
namespace Adyen.Management.Models
{
    ...
    public class MerchantJsonConverter : JsonConverter<Merchant>
    {
        ...
        public override Merchant Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (captureDelay.IsSet)
                merchant.CaptureDelay = captureDelay.Value;
            if (companyId.IsSet)
                merchant.CompanyId = companyId.Value;
            if (dataCenters.IsSet)
                merchant.DataCenters = dataCenters.Value;
            if (defaultShopperInteraction.IsSet)
                merchant.DefaultShopperInteraction = defaultShopperInteraction.Value;
            if (description.IsSet)
                merchant.Description = description.Value;
            if (id.IsSet)
                merchant.Id = id.Value;
            if (merchantCity.IsSet)
                merchant.MerchantCity = merchantCity.Value;
            if (name.IsSet)
                merchant.Name = name.Value;
            if (pricingPlan.IsSet)
                merchant.PricingPlan = pricingPlan.Value;
            if (primarySettlementCurrency.IsSet)
                merchant.PrimarySettlementCurrency = primarySettlementCurrency.Value;
            if (reference.IsSet)
                merchant.Reference = reference.Value;
            if (shopWebAddress.IsSet)
                merchant.ShopWebAddress = shopWebAddress.Value;
            if (status.IsSet)
