    ...
    public class AddressJsonConverter : JsonConverter<Address>
    {
        ...
        public override Address Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (companyName.IsSet)
                address.CompanyName = companyName.Value;
            if (country.IsSet)
                address.Country = country.Value;
            if (postalCode.IsSet)
                address.PostalCode = postalCode.Value;
            if (stateOrProvince.IsSet)
                address.StateOrProvince = stateOrProvince.Value;
            if (streetAddress.IsSet)
                address.StreetAddress = streetAddress.Value;
            if (streetAddress2.IsSet)
                address.StreetAddress2 = streetAddress2.Value;
            return address;
        }
