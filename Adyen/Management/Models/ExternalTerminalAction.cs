...
namespace Adyen.Management.Models
{
    ...
    /// </summary>
    [JsonConverter(typeof(ExternalTerminalActionJsonConverter))]
    ...
    public class ExternalTerminalActionJsonConverter : JsonConverter<ExternalTerminalAction>
    {
        ...
        public override ExternalTerminalAction Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (config.IsSet)
                externalTerminalAction.Config = config.Value;
            if (confirmedAt.IsSet)
                externalTerminalAction.ConfirmedAt = confirmedAt.Value;
            if (id.IsSet)
                externalTerminalAction.Id = id.Value;
            if (result.IsSet)
                externalTerminalAction.Result = result.Value;
            if (scheduledAt.IsSet)
                externalTerminalAction.ScheduledAt = scheduledAt.Value;
            if (status.IsSet)
                externalTerminalAction.Status = status.Value;
            if (terminalId.IsSet)
                externalTerminalAction.TerminalId = terminalId.Value;
