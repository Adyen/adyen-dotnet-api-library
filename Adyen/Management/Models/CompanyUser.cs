...
namespace Adyen.Management.Models
{
    ...
    /// CompanyUser.
    /// </summary>
    [JsonConverter(typeof(CompanyUserJsonConverter))]
    ...
    public class CompanyUserJsonConverter : JsonConverter<CompanyUser>
    {
        ...
        public override CompanyUser Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (accountGroups.IsSet)
                companyUser.AccountGroups = accountGroups.Value;
            if (active.IsSet)
                companyUser.Active = active.Value;
            if (apps.IsSet)
                companyUser.Apps = apps.Value;
            if (associatedMerchantAccounts.IsSet)
                companyUser.AssociatedMerchantAccounts = associatedMerchantAccounts.Value;
            if (name.IsSet)
                companyUser.Name = name.Value;
            return companyUser;
        }
        ...
        public void WriteProperties(Utf8JsonWriter writer, CompanyUser companyUser, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (companyUser._ActiveOption.IsSet)
                if (companyUser._ActiveOption.Value != null)
