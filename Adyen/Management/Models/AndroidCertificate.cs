...
namespace Adyen.Management.Models
{
    ...
    /// AndroidCertificate.
    /// </summary>
    [JsonConverter(typeof(AndroidCertificateJsonConverter))]
    ...
    public class AndroidCertificateJsonConverter : JsonConverter<AndroidCertificate>
    {
        ...
        public override AndroidCertificate Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (extension.IsSet)
                androidCertificate.Extension = extension.Value;
            if (name.IsSet)
                androidCertificate.Name = name.Value;
            if (notAfter.IsSet)
                androidCertificate.NotAfter = notAfter.Value;
            if (notBefore.IsSet)
                androidCertificate.NotBefore = notBefore.Value;
            if (status.IsSet)
                androidCertificate.Status = status.Value;
            return androidCertificate;
        }
