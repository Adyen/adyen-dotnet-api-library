...
namespace Adyen.Management.Models
{
    ...
    /// CustomNotification.
    /// </summary>
    [JsonConverter(typeof(CustomNotificationJsonConverter))]
    ...
    public class CustomNotificationJsonConverter : JsonConverter<CustomNotification>
    {
        ...
        public override CustomNotification Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (eventCode.IsSet)
                customNotification.EventCode = eventCode.Value;
            if (eventDate.IsSet)
                customNotification.EventDate = eventDate.Value;
            if (merchantReference.IsSet)
                customNotification.MerchantReference = merchantReference.Value;
            if (paymentMethod.IsSet)
                customNotification.PaymentMethod = paymentMethod.Value;
            if (reason.IsSet)
                customNotification.Reason = reason.Value;
            if (success.IsSet)
                customNotification.Success = success.Value;
