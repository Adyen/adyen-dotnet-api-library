...
namespace Adyen.Management.Models
{
    ...
    public class ProfileJsonConverter : JsonConverter<Profile>
    {
        ...
        public override Profile Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (defaultProfile.IsSet)
                profile.DefaultProfile = defaultProfile.Value;
            if (domainSuffix.IsSet)
                profile.DomainSuffix = domainSuffix.Value;
            if (eap.IsSet)
                profile.Eap = eap.Value;
            if (eapCaCert.IsSet)
                profile.EapCaCert = eapCaCert.Value;
            if (eapClientCert.IsSet)
                profile.EapClientCert = eapClientCert.Value;
            if (eapClientKey.IsSet)
                profile.EapClientKey = eapClientKey.Value;
            if (eapClientPwd.IsSet)
                profile.EapClientPwd = eapClientPwd.Value;
            if (eapIdentity.IsSet)
                profile.EapIdentity = eapIdentity.Value;
            if (eapIntermediateCert.IsSet)
                profile.EapIntermediateCert = eapIntermediateCert.Value;
            if (eapPwd.IsSet)
                profile.EapPwd = eapPwd.Value;
            if (hiddenSsid.IsSet)
                profile.HiddenSsid = hiddenSsid.Value;
            if (name.IsSet)
                profile.Name = name.Value;
            if (psk.IsSet)
                profile.Psk = psk.Value;
            return profile;
        }
