...
namespace Adyen.Management.Models
{
    ...
    /// ReceiptPrinting.
    /// </summary>
    [JsonConverter(typeof(ReceiptPrintingJsonConverter))]
    ...
    public class ReceiptPrintingJsonConverter : JsonConverter<ReceiptPrinting>
    {
        ...
        public override ReceiptPrinting Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (merchantCancelled.IsSet)
                receiptPrinting.MerchantCancelled = merchantCancelled.Value;
            if (merchantCaptureApproved.IsSet)
                receiptPrinting.MerchantCaptureApproved = merchantCaptureApproved.Value;
            if (merchantCaptureRefused.IsSet)
                receiptPrinting.MerchantCaptureRefused = merchantCaptureRefused.Value;
            if (merchantRefundApproved.IsSet)
                receiptPrinting.MerchantRefundApproved = merchantRefundApproved.Value;
            if (merchantRefundRefused.IsSet)
                receiptPrinting.MerchantRefundRefused = merchantRefundRefused.Value;
            if (merchantRefused.IsSet)
                receiptPrinting.MerchantRefused = merchantRefused.Value;
            if (merchantVoid.IsSet)
                receiptPrinting.MerchantVoid = merchantVoid.Value;
            if (shopperApproved.IsSet)
                receiptPrinting.ShopperApproved = shopperApproved.Value;
            if (shopperCancelled.IsSet)
                receiptPrinting.ShopperCancelled = shopperCancelled.Value;
            if (shopperCaptureApproved.IsSet)
                receiptPrinting.ShopperCaptureApproved = shopperCaptureApproved.Value;
            if (shopperCaptureRefused.IsSet)
                receiptPrinting.ShopperCaptureRefused = shopperCaptureRefused.Value;
            if (shopperRefundApproved.IsSet)
                receiptPrinting.ShopperRefundApproved = shopperRefundApproved.Value;
            if (shopperRefundRefused.IsSet)
                receiptPrinting.ShopperRefundRefused = shopperRefundRefused.Value;
            if (shopperRefused.IsSet)
                receiptPrinting.ShopperRefused = shopperRefused.Value;
            if (shopperVoid.IsSet)
                receiptPrinting.ShopperVoid = shopperVoid.Value;
