...
namespace Adyen.StoredValue.Models
{
    ...
    /// </summary>
    [JsonConverter(typeof(StoredValueIssueResponseJsonConverter))]
    ...
    public class StoredValueIssueResponseJsonConverter : JsonConverter<StoredValueIssueResponse>
    {
        ...
        public override StoredValueIssueResponse Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            var storedValueIssueResponse = new StoredValueIssueResponse();
            if (authCode.IsSet)
                storedValueIssueResponse.AuthCode = authCode.Value;
            if (currentBalance.IsSet)
                storedValueIssueResponse.CurrentBalance = currentBalance.Value;
            if (paymentMethod.IsSet)
                storedValueIssueResponse.PaymentMethod = paymentMethod.Value;
            if (pspReference.IsSet)
                storedValueIssueResponse.PspReference = pspReference.Value;
            if (refusalReason.IsSet)
                storedValueIssueResponse.RefusalReason = refusalReason.Value;
            if (resultCode.IsSet)
                storedValueIssueResponse.ResultCode = resultCode.Value;
            if (thirdPartyRefusalReason.IsSet)
                storedValueIssueResponse.ThirdPartyRefusalReason = thirdPartyRefusalReason.Value;
