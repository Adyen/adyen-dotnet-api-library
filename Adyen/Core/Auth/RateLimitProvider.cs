...
namespace Adyen.Core.Auth
{
    ...
    public class RateLimitProvider<TTokenBase> : TokenProvider<TTokenBase> where TTokenBase : TokenBase
    {
        ...
        public RateLimitProvider(TokenContainer<TTokenBase> container) : base(container.Tokens)
        {
            ...
            {
                string[] headers = apiKeyTokenContainer.Tokens.Select(t => t.Key).Distinct().ToArray();

                foreach (string header in headers)
                {
                    global::System.Threading.Channels.BoundedChannelOptions options = new global::System.Threading.Channels.BoundedChannelOptions(apiKeyTokenContainer.Tokens.Count(t => t.Key.Equals(header)))
                    {
            ...
            foreach (var availableToken in AvailableTokens)
                foreach(TTokenBase token in _tokens)
                {
                    ...
                    {
                        if (apiKeyToken.Key == availableToken.Key)
                        {
