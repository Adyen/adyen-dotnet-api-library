...
namespace Adyen.ConfigurationWebhooks.Models
{
    ...
    /// CardOrderItem.
    /// </summary>
    [JsonConverter(typeof(CardOrderItemJsonConverter))]
    ...
    public class CardOrderItemJsonConverter : JsonConverter<CardOrderItem>
    {
        ...
        public override CardOrderItem Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (card.IsSet)
                cardOrderItem.Card = card.Value;
            if (cardOrderItemId.IsSet)
                cardOrderItem.CardOrderItemId = cardOrderItemId.Value;
            if (creationDate.IsSet)
                cardOrderItem.CreationDate = creationDate.Value;
            if (id.IsSet)
                cardOrderItem.Id = id.Value;
            if (paymentInstrumentId.IsSet)
                cardOrderItem.PaymentInstrumentId = paymentInstrumentId.Value;
            if (pin.IsSet)
                cardOrderItem.Pin = pin.Value;
            if (shippingMethod.IsSet)
                cardOrderItem.ShippingMethod = shippingMethod.Value;
