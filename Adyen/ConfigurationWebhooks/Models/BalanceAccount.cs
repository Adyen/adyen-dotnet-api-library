...
namespace Adyen.ConfigurationWebhooks.Models
{
    ...
    /// BalanceAccount.
    /// </summary>
    [JsonConverter(typeof(BalanceAccountJsonConverter))]
    ...
    public class BalanceAccountJsonConverter : JsonConverter<BalanceAccount>
    {
        ...
        public override BalanceAccount Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
                ...
                if (utf8JsonReader.TokenType == JsonTokenType.PropertyName && currentDepth == utf8JsonReader.CurrentDepth - 1)
                {
                    ...
                    switch (jsonPropertyName)
                    {
                        ...
                        case "status":
                            ...
                            status = new Option<BalanceAccount.StatusEnum?>(BalanceAccount.StatusEnum.FromStringOrDefault(statusRawValue) ?? (BalanceAccount.StatusEnum)statusRawValue);
                            break;
                        case "timeZone":
            ...
            if (defaultCurrencyCode.IsSet)
                balanceAccount.DefaultCurrencyCode = defaultCurrencyCode.Value;
            if (description.IsSet)
                balanceAccount.Description = description.Value;
            if (metadata.IsSet)
                balanceAccount.Metadata = metadata.Value;
            if (migratedAccountCode.IsSet)
                balanceAccount.MigratedAccountCode = migratedAccountCode.Value;
            if (platformPaymentConfiguration.IsSet)
                balanceAccount.PlatformPaymentConfiguration = platformPaymentConfiguration.Value;
            if (reference.IsSet)
                balanceAccount.Reference = reference.Value;
            if (status.IsSet)
                balanceAccount.Status = status.Value;
            if (timeZone.IsSet)
                balanceAccount.TimeZone = timeZone.Value;
