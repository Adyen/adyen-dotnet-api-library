...
namespace Adyen.ConfigurationWebhooks.Models
{
    ...
    public class NetworkTokenNotificationDataV2JsonConverter : JsonConverter<NetworkTokenNotificationDataV2>
    {
        ...
        public override NetworkTokenNotificationDataV2 Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (authenticationApplied.IsSet)
                networkTokenNotificationDataV2.AuthenticationApplied = authenticationApplied.Value;
            if (balancePlatform.IsSet)
                networkTokenNotificationDataV2.BalancePlatform = balancePlatform.Value;
            if (decision.IsSet)
                networkTokenNotificationDataV2.Decision = decision.Value;
            if (id.IsSet)
                networkTokenNotificationDataV2.Id = id.Value;
            if (paymentInstrumentId.IsSet)
                networkTokenNotificationDataV2.PaymentInstrumentId = paymentInstrumentId.Value;
            if (schemeRiskScore.IsSet)
                networkTokenNotificationDataV2.SchemeRiskScore = schemeRiskScore.Value;
            if (status.IsSet)
                networkTokenNotificationDataV2.Status = status.Value;
            if (tokenLastFour.IsSet)
                networkTokenNotificationDataV2.TokenLastFour = tokenLastFour.Value;
            if (tokenRequestor.IsSet)
                networkTokenNotificationDataV2.TokenRequestor = tokenRequestor.Value;
            if (transactionRulesResult.IsSet)
                networkTokenNotificationDataV2.TransactionRulesResult = transactionRulesResult.Value;
            if (type.IsSet)
                networkTokenNotificationDataV2.Type = type.Value;
            if (validationFacts.IsSet)
                networkTokenNotificationDataV2.ValidationFacts = validationFacts.Value;
            if (wallet.IsSet)
                networkTokenNotificationDataV2.Wallet = wallet.Value;
            return networkTokenNotificationDataV2;
        }
