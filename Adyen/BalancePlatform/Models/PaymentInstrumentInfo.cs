...
namespace Adyen.BalancePlatform.Models
{
    ...
    /// </summary>
    [JsonConverter(typeof(PaymentInstrumentInfoJsonConverter))]
    ...
    public class PaymentInstrumentInfoJsonConverter : JsonConverter<PaymentInstrumentInfo>
    {
        ...
        public override PaymentInstrumentInfo Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (card.IsSet)
                paymentInstrumentInfo.Card = card.Value;
            if (description.IsSet)
                paymentInstrumentInfo.Description = description.Value;
            if (paymentInstrumentGroupId.IsSet)
                paymentInstrumentInfo.PaymentInstrumentGroupId = paymentInstrumentGroupId.Value;
            if (reference.IsSet)
                paymentInstrumentInfo.Reference = reference.Value;
            if (status.IsSet)
                paymentInstrumentInfo.Status = status.Value;
            if (statusComment.IsSet)
                paymentInstrumentInfo.StatusComment = statusComment.Value;
            if (statusReason.IsSet)
                paymentInstrumentInfo.StatusReason = statusReason.Value;
