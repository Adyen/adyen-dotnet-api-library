...
namespace Adyen.BalancePlatform.Models
{
    ...
    /// </summary>
    [JsonConverter(typeof(SweepConfigurationV2JsonConverter))]
    ...
    public class SweepConfigurationV2JsonConverter : JsonConverter<SweepConfigurationV2>
    {
        ...
        public override SweepConfigurationV2 Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (description.IsSet)
                sweepConfigurationV2.Description = description.Value;
            if (priorities.IsSet)
                sweepConfigurationV2.Priorities = priorities.Value;
            if (reason.IsSet)
                sweepConfigurationV2.Reason = reason.Value;
            if (reasonDetail.IsSet)
                sweepConfigurationV2.ReasonDetail = reasonDetail.Value;
            if (reference.IsSet)
                sweepConfigurationV2.Reference = reference.Value;
            if (referenceForBeneficiary.IsSet)
                sweepConfigurationV2.ReferenceForBeneficiary = referenceForBeneficiary.Value;
            if (status.IsSet)
                sweepConfigurationV2.Status = status.Value;
            if (sweepAmount.IsSet)
                sweepConfigurationV2.SweepAmount = sweepAmount.Value;
            if (targetAmount.IsSet)
                sweepConfigurationV2.TargetAmount = targetAmount.Value;
            if (triggerAmount.IsSet)
                sweepConfigurationV2.TriggerAmount = triggerAmount.Value;
            if (type.IsSet)
                sweepConfigurationV2.Type = type.Value;
