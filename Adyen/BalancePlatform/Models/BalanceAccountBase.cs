...
namespace Adyen.BalancePlatform.Models
{
    ...
    /// BalanceAccountBase.
    /// </summary>
    [JsonConverter(typeof(BalanceAccountBaseJsonConverter))]
    public partial class BalanceAccountBase
    {
        /// <summary>
    ...
    public class BalanceAccountBaseJsonConverter : JsonConverter<BalanceAccountBase>
    {
        ...
        public override BalanceAccountBase Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
                ...
                if (utf8JsonReader.TokenType == JsonTokenType.PropertyName && currentDepth == utf8JsonReader.CurrentDepth - 1)
                {
                    ...
                    switch (jsonPropertyName)
                    {
                        ...
                        case "status":
                            ...
                            status = new Option<BalanceAccountBase.StatusEnum?>(BalanceAccountBase.StatusEnum.FromStringOrDefault(statusRawValue) ?? (BalanceAccountBase.StatusEnum)statusRawValue);
                            break;
                        case "timeZone":
            ...
            var balanceAccountBase = new BalanceAccountBase();
            if (accountHolderId.IsSet)
                balanceAccountBase.AccountHolderId = accountHolderId.Value!;
            if (id.IsSet)
                balanceAccountBase.Id = id.Value!;
            if (defaultCurrencyCode.IsSet)
                balanceAccountBase.DefaultCurrencyCode = defaultCurrencyCode.Value;
            if (description.IsSet)
                balanceAccountBase.Description = description.Value;
            if (metadata.IsSet)
                balanceAccountBase.Metadata = metadata.Value;
            if (migratedAccountCode.IsSet)
                balanceAccountBase.MigratedAccountCode = migratedAccountCode.Value;
            if (platformPaymentConfiguration.IsSet)
                balanceAccountBase.PlatformPaymentConfiguration = platformPaymentConfiguration.Value;
            if (reference.IsSet)
                balanceAccountBase.Reference = reference.Value;
            if (status.IsSet)
                balanceAccountBase.Status = status.Value;
            if (timeZone.IsSet)
                balanceAccountBase.TimeZone = timeZone.Value;
