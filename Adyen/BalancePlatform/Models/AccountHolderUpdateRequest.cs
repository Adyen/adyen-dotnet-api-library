...
namespace Adyen.BalancePlatform.Models
{
    ...
    /// </summary>
    [JsonConverter(typeof(AccountHolderUpdateRequestJsonConverter))]
    ...
    public class AccountHolderUpdateRequestJsonConverter : JsonConverter<AccountHolderUpdateRequest>
    {
        ...
        public override AccountHolderUpdateRequest Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
                ...
                if (utf8JsonReader.TokenType == JsonTokenType.PropertyName && currentDepth == utf8JsonReader.CurrentDepth - 1)
                {
                    ...
                    switch (jsonPropertyName)
                    {
                        ...
                        case "status":
                            ...
                            status = new Option<AccountHolderUpdateRequest.StatusEnum?>(AccountHolderUpdateRequest.StatusEnum.FromStringOrDefault(statusRawValue) ?? (AccountHolderUpdateRequest.StatusEnum)statusRawValue);
                            break;
                        case "timeZone":
            ...
            if (capabilities.IsSet)
                accountHolderUpdateRequest.Capabilities = capabilities.Value;
            if (contactDetails.IsSet)
                accountHolderUpdateRequest.ContactDetails = contactDetails.Value;
            if (description.IsSet)
                accountHolderUpdateRequest.Description = description.Value;
            if (metadata.IsSet)
                accountHolderUpdateRequest.Metadata = metadata.Value;
            if (migratedAccountHolderCode.IsSet)
                accountHolderUpdateRequest.MigratedAccountHolderCode = migratedAccountHolderCode.Value;
            if (primaryBalanceAccount.IsSet)
                accountHolderUpdateRequest.PrimaryBalanceAccount = primaryBalanceAccount.Value;
            if (reference.IsSet)
                accountHolderUpdateRequest.Reference = reference.Value;
            if (status.IsSet)
                accountHolderUpdateRequest.Status = status.Value;
            if (timeZone.IsSet)
                accountHolderUpdateRequest.TimeZone = timeZone.Value;
            if (verificationDeadlines.IsSet)
                accountHolderUpdateRequest.VerificationDeadlines = verificationDeadlines.Value;
            return accountHolderUpdateRequest;
        }
