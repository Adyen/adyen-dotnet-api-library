...
namespace Adyen.BalancePlatform.Models
{
    ...
    /// AccountHolder.
    /// </summary>
    [JsonConverter(typeof(AccountHolderJsonConverter))]
    ...
    public class AccountHolderJsonConverter : JsonConverter<AccountHolder>
    {
        ...
        public override AccountHolder Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
                ...
                if (utf8JsonReader.TokenType == JsonTokenType.PropertyName && currentDepth == utf8JsonReader.CurrentDepth - 1)
                {
                    ...
                    switch (jsonPropertyName)
                    {
                        ...
                        case "status":
                            ...
                            status = new Option<AccountHolder.StatusEnum?>(AccountHolder.StatusEnum.FromStringOrDefault(statusRawValue) ?? (AccountHolder.StatusEnum)statusRawValue);
                            break;
                        case "timeZone":
            ...
            if (capabilities.IsSet)
                accountHolder.Capabilities = capabilities.Value;
            if (contactDetails.IsSet)
                accountHolder.ContactDetails = contactDetails.Value;
            if (description.IsSet)
                accountHolder.Description = description.Value;
            if (metadata.IsSet)
                accountHolder.Metadata = metadata.Value;
            if (migratedAccountHolderCode.IsSet)
                accountHolder.MigratedAccountHolderCode = migratedAccountHolderCode.Value;
            if (primaryBalanceAccount.IsSet)
                accountHolder.PrimaryBalanceAccount = primaryBalanceAccount.Value;
            if (reference.IsSet)
                accountHolder.Reference = reference.Value;
            if (status.IsSet)
                accountHolder.Status = status.Value;
            if (timeZone.IsSet)
                accountHolder.TimeZone = timeZone.Value;
            if (verificationDeadlines.IsSet)
                accountHolder.VerificationDeadlines = verificationDeadlines.Value;
