    ...
    public class AccountHolderInfoJsonConverter : JsonConverter<AccountHolderInfo>
    {
        ...
        public override AccountHolderInfo Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (capabilities.IsSet)
                accountHolderInfo.Capabilities = capabilities.Value;
            if (contactDetails.IsSet)
                accountHolderInfo.ContactDetails = contactDetails.Value;
            if (description.IsSet)
                accountHolderInfo.Description = description.Value;
            if (metadata.IsSet)
                accountHolderInfo.Metadata = metadata.Value;
            if (migratedAccountHolderCode.IsSet)
                accountHolderInfo.MigratedAccountHolderCode = migratedAccountHolderCode.Value;
            if (reference.IsSet)
                accountHolderInfo.Reference = reference.Value;
            if (timeZone.IsSet)
                accountHolderInfo.TimeZone = timeZone.Value;
