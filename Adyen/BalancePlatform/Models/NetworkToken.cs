...
namespace Adyen.BalancePlatform.Models
{
    ...
    /// NetworkToken.
    /// </summary>
    [JsonConverter(typeof(NetworkTokenJsonConverter))]
    ...
    public class NetworkTokenJsonConverter : JsonConverter<NetworkToken>
    {
        ...
        public override NetworkToken Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (creationDate.IsSet)
                networkToken.CreationDate = creationDate.Value;
            if (device.IsSet)
                networkToken.Device = device.Value;
            if (id.IsSet)
                networkToken.Id = id.Value;
            if (paymentInstrumentId.IsSet)
                networkToken.PaymentInstrumentId = paymentInstrumentId.Value;
            if (status.IsSet)
                networkToken.Status = status.Value;
            if (tokenLastFour.IsSet)
                networkToken.TokenLastFour = tokenLastFour.Value;
            if (tokenRequestor.IsSet)
                networkToken.TokenRequestor = tokenRequestor.Value;
            if (type.IsSet)
                networkToken.Type = type.Value;
            return networkToken;
        }
