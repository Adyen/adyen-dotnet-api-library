...
namespace Adyen.Payment.Models
{
    ...
    /// PaymentResult.
    /// </summary>
    [JsonConverter(typeof(PaymentResultJsonConverter))]
    ...
    public class PaymentResultJsonConverter : JsonConverter<PaymentResult>
    {
        ...
        public override PaymentResult Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
                ...
                if (utf8JsonReader.TokenType == JsonTokenType.PropertyName && currentDepth == utf8JsonReader.CurrentDepth - 1)
                {
                    ...
                    switch (jsonPropertyName)
                    {
                        ...
                        case "resultCode":
                            ...
                            resultCode = new Option<PaymentResult.ResultCodeEnum?>(PaymentResult.ResultCodeEnum.FromStringOrDefault(resultCodeRawValue) ?? (PaymentResult.ResultCodeEnum)resultCodeRawValue);
                            break;
                        default:
                            break;
                    }
            ...
            if (authCode.IsSet)
                paymentResult.AuthCode = authCode.Value;
            if (dccAmount.IsSet)
                paymentResult.DccAmount = dccAmount.Value;
            if (dccSignature.IsSet)
                paymentResult.DccSignature = dccSignature.Value;
            if (fraudResult.IsSet)
                paymentResult.FraudResult = fraudResult.Value;
            if (issuerUrl.IsSet)
                paymentResult.IssuerUrl = issuerUrl.Value;
            if (md.IsSet)
                paymentResult.Md = md.Value;
            if (paRequest.IsSet)
                paymentResult.PaRequest = paRequest.Value;
            if (pspReference.IsSet)
                paymentResult.PspReference = pspReference.Value;
            if (refusalReason.IsSet)
                paymentResult.RefusalReason = refusalReason.Value;
            if (resultCode.IsSet)
                paymentResult.ResultCode = resultCode.Value;
