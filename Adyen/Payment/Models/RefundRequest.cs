...
namespace Adyen.Payment.Models
{
    ...
    public class RefundRequestJsonConverter : JsonConverter<RefundRequest>
    {
        ...
        public override RefundRequest Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (mpiData.IsSet)
                refundRequest.MpiData = mpiData.Value;
            if (originalMerchantReference.IsSet)
                refundRequest.OriginalMerchantReference = originalMerchantReference.Value;
            if (platformChargebackLogic.IsSet)
                refundRequest.PlatformChargebackLogic = platformChargebackLogic.Value;
            if (reference.IsSet)
                refundRequest.Reference = reference.Value;
            if (splits.IsSet)
                refundRequest.Splits = splits.Value;
            if (tenderReference.IsSet)
                refundRequest.TenderReference = tenderReference.Value;
            if (uniqueTerminalId.IsSet)
                refundRequest.UniqueTerminalId = uniqueTerminalId.Value;
