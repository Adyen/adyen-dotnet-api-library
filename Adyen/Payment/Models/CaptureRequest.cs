...
namespace Adyen.Payment.Models
{
    ...
    /// CaptureRequest.
    /// </summary>
    [JsonConverter(typeof(CaptureRequestJsonConverter))]
    ...
    public class CaptureRequestJsonConverter : JsonConverter<CaptureRequest>
    {
        ...
        public override CaptureRequest Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            captureRequest.MerchantAccount = merchantAccount.Value!;
            captureRequest.ModificationAmount = modificationAmount.Value!;
            captureRequest.OriginalReference = originalReference.Value!;
            if (additionalData.IsSet)
                captureRequest.AdditionalData = additionalData.Value;
            if (mpiData.IsSet)
                captureRequest.MpiData = mpiData.Value;
            if (originalMerchantReference.IsSet)
                captureRequest.OriginalMerchantReference = originalMerchantReference.Value;
            if (platformChargebackLogic.IsSet)
                captureRequest.PlatformChargebackLogic = platformChargebackLogic.Value;
            if (reference.IsSet)
                captureRequest.Reference = reference.Value;
            if (splits.IsSet)
                captureRequest.Splits = splits.Value;
            if (tenderReference.IsSet)
                captureRequest.TenderReference = tenderReference.Value;
            if (uniqueTerminalId.IsSet)
                captureRequest.UniqueTerminalId = uniqueTerminalId.Value;
        ...
        public void WriteProperties(Utf8JsonWriter writer, CaptureRequest captureRequest, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (captureRequest._OriginalMerchantReferenceOption.IsSet)
                if (captureRequest.OriginalMerchantReference != null)
            ...
            if (captureRequest._ReferenceOption.IsSet)
                if (captureRequest.Reference != null)
