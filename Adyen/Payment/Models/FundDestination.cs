...
namespace Adyen.Payment.Models
{
    ...
    /// FundDestination.
    /// </summary>
    [JsonConverter(typeof(FundDestinationJsonConverter))]
    ...
    public class FundDestinationJsonConverter : JsonConverter<FundDestination>
    {
        ...
        public override FundDestination Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (additionalData.IsSet)
                fundDestination.AdditionalData = additionalData.Value;
            if (billingAddress.IsSet)
                fundDestination.BillingAddress = billingAddress.Value;
            if (card.IsSet)
                fundDestination.Card = card.Value;
            if (selectedRecurringDetailReference.IsSet)
                fundDestination.SelectedRecurringDetailReference = selectedRecurringDetailReference.Value;
            if (shopperEmail.IsSet)
                fundDestination.ShopperEmail = shopperEmail.Value;
            if (shopperName.IsSet)
                fundDestination.ShopperName = shopperName.Value;
            if (shopperReference.IsSet)
                fundDestination.ShopperReference = shopperReference.Value;
            if (subMerchant.IsSet)
                fundDestination.SubMerchant = subMerchant.Value;
            if (telephoneNumber.IsSet)
                fundDestination.TelephoneNumber = telephoneNumber.Value;
            if (walletPurpose.IsSet)
                fundDestination.WalletPurpose = walletPurpose.Value;
