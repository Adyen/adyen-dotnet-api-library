...
namespace Adyen.Payment.Models
{
    ...
    /// CancelRequest.
    /// </summary>
    [JsonConverter(typeof(CancelRequestJsonConverter))]
    ...
    public class CancelRequestJsonConverter : JsonConverter<CancelRequest>
    {
        ...
        public override CancelRequest Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            cancelRequest.MerchantAccount = merchantAccount.Value!;
            cancelRequest.OriginalReference = originalReference.Value!;
            if (additionalData.IsSet)
                cancelRequest.AdditionalData = additionalData.Value;
            if (mpiData.IsSet)
                cancelRequest.MpiData = mpiData.Value;
            if (originalMerchantReference.IsSet)
                cancelRequest.OriginalMerchantReference = originalMerchantReference.Value;
            if (platformChargebackLogic.IsSet)
                cancelRequest.PlatformChargebackLogic = platformChargebackLogic.Value;
            if (reference.IsSet)
                cancelRequest.Reference = reference.Value;
            if (splits.IsSet)
                cancelRequest.Splits = splits.Value;
            if (tenderReference.IsSet)
                cancelRequest.TenderReference = tenderReference.Value;
            if (uniqueTerminalId.IsSet)
                cancelRequest.UniqueTerminalId = uniqueTerminalId.Value;
        ...
        public void WriteProperties(Utf8JsonWriter writer, CancelRequest cancelRequest, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (cancelRequest._OriginalMerchantReferenceOption.IsSet)
                if (cancelRequest.OriginalMerchantReference != null)
            ...
            if (cancelRequest._ReferenceOption.IsSet)
                if (cancelRequest.Reference != null)
