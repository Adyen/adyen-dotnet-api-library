    ...
    public class MandateJsonConverter : JsonConverter<Mandate>
    {
        ...
        public override Mandate Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (billingAttemptsRule.IsSet)
                mandate.BillingAttemptsRule = billingAttemptsRule.Value;
            if (billingDay.IsSet)
                mandate.BillingDay = billingDay.Value;
            if (count.IsSet)
                mandate.Count = count.Value;
            if (remarks.IsSet)
                mandate.Remarks = remarks.Value;
            if (startsAt.IsSet)
                mandate.StartsAt = startsAt.Value;
            return mandate;
        }
