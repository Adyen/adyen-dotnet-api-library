...
namespace Adyen.ManagementWebhooks.Models
{
    ...
    /// </summary>
    [JsonConverter(typeof(AccountCapabilityDataJsonConverter))]
    ...
    public class AccountCapabilityDataJsonConverter : JsonConverter<AccountCapabilityData>
    {
        ...
        public override AccountCapabilityData Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (allowedLevel.IsSet)
                accountCapabilityData.AllowedLevel = allowedLevel.Value;
            if (capability.IsSet)
                accountCapabilityData.Capability = capability.Value;
            if (problems.IsSet)
                accountCapabilityData.Problems = problems.Value;
            if (verificationDeadline.IsSet)
                accountCapabilityData.VerificationDeadline = verificationDeadline.Value;
            if (verificationStatus.IsSet)
                accountCapabilityData.VerificationStatus = verificationStatus.Value;
            return accountCapabilityData;
        }
