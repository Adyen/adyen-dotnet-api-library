...
namespace Adyen.RelayedAuthorizationWebhooks.Models
{
    ...
    /// BankAccountDetails.
    /// </summary>
    [JsonConverter(typeof(BankAccountDetailsJsonConverter))]
    ...
    public class BankAccountDetailsJsonConverter : JsonConverter<BankAccountDetails>
    {
        ...
        public override BankAccountDetails Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (accountNumber.IsSet)
                bankAccountDetails.AccountNumber = accountNumber.Value;
            if (accountType.IsSet)
                bankAccountDetails.AccountType = accountType.Value;
            if (branchNumber.IsSet)
                bankAccountDetails.BranchNumber = branchNumber.Value;
            if (formFactor.IsSet)
                bankAccountDetails.FormFactor = formFactor.Value;
            if (iban.IsSet)
                bankAccountDetails.Iban = iban.Value;
