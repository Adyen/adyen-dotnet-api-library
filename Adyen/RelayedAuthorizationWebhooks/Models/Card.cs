    ...
    public class CardJsonConverter : JsonConverter<Card>
    {
        ...
        public override Card Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (configuration.IsSet)
                card.Configuration = configuration.Value;
            if (cvc.IsSet)
                card.Cvc = cvc.Value;
            if (deliveryContact.IsSet)
                card.DeliveryContact = deliveryContact.Value;
            if (expiration.IsSet)
                card.Expiration = expiration.Value;
            if (lastFour.IsSet)
                card.LastFour = lastFour.Value;
            if (number.IsSet)
                card.Number = number.Value;
            if (threeDSecure.IsSet)
                card.ThreeDSecure = threeDSecure.Value;
            if (usage.IsSet)
                card.Usage = usage.Value;
            return card;
        }
