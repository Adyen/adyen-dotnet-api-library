...
namespace Adyen.RelayedAuthorizationWebhooks.Models
{
    ...
    /// BulkAddress.
    /// </summary>
    [JsonConverter(typeof(BulkAddressJsonConverter))]
    ...
    public class BulkAddressJsonConverter : JsonConverter<BulkAddress>
    {
        ...
        public override BulkAddress Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (company.IsSet)
                bulkAddress.Company = company.Value;
            if (email.IsSet)
                bulkAddress.Email = email.Value;
            if (houseNumberOrName.IsSet)
                bulkAddress.HouseNumberOrName = houseNumberOrName.Value;
            if (line1.IsSet)
                bulkAddress.Line1 = line1.Value;
            if (line2.IsSet)
                bulkAddress.Line2 = line2.Value;
            if (line3.IsSet)
                bulkAddress.Line3 = line3.Value;
            if (mobile.IsSet)
                bulkAddress.Mobile = mobile.Value;
            if (name.IsSet)
                bulkAddress.Name = name.Value;
            if (postalCode.IsSet)
                bulkAddress.PostalCode = postalCode.Value;
            if (stateOrProvince.IsSet)
                bulkAddress.StateOrProvince = stateOrProvince.Value;
            if (street.IsSet)
                bulkAddress.Street = street.Value;
            return bulkAddress;
        }
