    ...
    public class PaymentInstrumentJsonConverter : JsonConverter<PaymentInstrument>
    {
        ...
        public override PaymentInstrument Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (bankAccount.IsSet)
                paymentInstrument.BankAccount = bankAccount.Value;
            if (card.IsSet)
                paymentInstrument.Card = card.Value;
            if (description.IsSet)
                paymentInstrument.Description = description.Value;
            if (paymentInstrumentGroupId.IsSet)
                paymentInstrument.PaymentInstrumentGroupId = paymentInstrumentGroupId.Value;
            if (reference.IsSet)
                paymentInstrument.Reference = reference.Value;
            if (replacedById.IsSet)
                paymentInstrument.ReplacedById = replacedById.Value;
            if (replacementOfId.IsSet)
                paymentInstrument.ReplacementOfId = replacementOfId.Value;
            if (status.IsSet)
                paymentInstrument.Status = status.Value;
            if (statusComment.IsSet)
                paymentInstrument.StatusComment = statusComment.Value;
            if (statusReason.IsSet)
                paymentInstrument.StatusReason = statusReason.Value;
