...
namespace Adyen.Checkout.Models
{
    ...
    /// ApplicationInfo.
    /// </summary>
    [JsonConverter(typeof(ApplicationInfoJsonConverter))]
    ...
    public class ApplicationInfoJsonConverter : JsonConverter<ApplicationInfo>
    {
        ...
        public override ApplicationInfo Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (adyenPaymentSource.IsSet)
                applicationInfo.AdyenPaymentSource = adyenPaymentSource.Value;
            if (externalPlatform.IsSet)
                applicationInfo.ExternalPlatform = externalPlatform.Value;
            if (merchantApplication.IsSet)
                applicationInfo.MerchantApplication = merchantApplication.Value;
            if (merchantDevice.IsSet)
                applicationInfo.MerchantDevice = merchantDevice.Value;
            if (shopperInteractionDevice.IsSet)
                applicationInfo.ShopperInteractionDevice = shopperInteractionDevice.Value;
