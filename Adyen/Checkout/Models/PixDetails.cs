...
namespace Adyen.Checkout.Models
{
    ...
    /// PixDetails.
    /// </summary>
    [JsonConverter(typeof(PixDetailsJsonConverter))]
    ...
    public class PixDetailsJsonConverter : JsonConverter<PixDetails>
    {
        ...
        public override PixDetails Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (pixRecurring.IsSet)
                pixDetails.PixRecurring = pixRecurring.Value;
            if (recurringDetailReference.IsSet)
                pixDetails.RecurringDetailReference = recurringDetailReference.Value;
            if (sdkData.IsSet)
                pixDetails.SdkData = sdkData.Value;
            if (storedPaymentMethodId.IsSet)
                pixDetails.StoredPaymentMethodId = storedPaymentMethodId.Value;
            if (type.IsSet)
                pixDetails.Type = type.Value;
            return pixDetails;
        }
