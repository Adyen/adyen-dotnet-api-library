...
namespace Adyen.Checkout.Models
{
    ...
    /// </summary>
    [JsonConverter(typeof(ThreeDS2ResponseDataJsonConverter))]
    ...
    public class ThreeDS2ResponseDataJsonConverter : JsonConverter<ThreeDS2ResponseData>
    {
        ...
        public override ThreeDS2ResponseData Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (acsOperatorID.IsSet)
                threeDS2ResponseData.AcsOperatorID = acsOperatorID.Value;
            if (acsReferenceNumber.IsSet)
                threeDS2ResponseData.AcsReferenceNumber = acsReferenceNumber.Value;
            if (acsSignedContent.IsSet)
                threeDS2ResponseData.AcsSignedContent = acsSignedContent.Value;
            if (acsTransID.IsSet)
                threeDS2ResponseData.AcsTransID = acsTransID.Value;
            if (acsURL.IsSet)
                threeDS2ResponseData.AcsURL = acsURL.Value;
            if (authenticationType.IsSet)
                threeDS2ResponseData.AuthenticationType = authenticationType.Value;
            if (cardHolderInfo.IsSet)
                threeDS2ResponseData.CardHolderInfo = cardHolderInfo.Value;
            if (cavvAlgorithm.IsSet)
                threeDS2ResponseData.CavvAlgorithm = cavvAlgorithm.Value;
            if (challengeIndicator.IsSet)
                threeDS2ResponseData.ChallengeIndicator = challengeIndicator.Value;
            if (dsReferenceNumber.IsSet)
                threeDS2ResponseData.DsReferenceNumber = dsReferenceNumber.Value;
            if (dsTransID.IsSet)
                threeDS2ResponseData.DsTransID = dsTransID.Value;
            if (exemptionIndicator.IsSet)
                threeDS2ResponseData.ExemptionIndicator = exemptionIndicator.Value;
            if (messageVersion.IsSet)
                threeDS2ResponseData.MessageVersion = messageVersion.Value;
            if (riskScore.IsSet)
                threeDS2ResponseData.RiskScore = riskScore.Value;
            if (sdkEphemPubKey.IsSet)
                threeDS2ResponseData.SdkEphemPubKey = sdkEphemPubKey.Value;
            if (threeDSServerTransID.IsSet)
                threeDS2ResponseData.ThreeDSServerTransID = threeDSServerTransID.Value;
            if (transStatus.IsSet)
                threeDS2ResponseData.TransStatus = transStatus.Value;
            if (transStatusReason.IsSet)
                threeDS2ResponseData.TransStatusReason = transStatusReason.Value;
