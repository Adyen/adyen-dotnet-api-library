...
namespace Adyen.Checkout.Models
{
    ...
    public class PaymentResponseJsonConverter : JsonConverter<PaymentResponse>
    {
        ...
        public override PaymentResponse Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (action.IsSet)
                paymentResponse.Action = action.Value;
            if (additionalData.IsSet)
                paymentResponse.AdditionalData = additionalData.Value;
            if (amount.IsSet)
                paymentResponse.Amount = amount.Value;
            if (donationToken.IsSet)
                paymentResponse.DonationToken = donationToken.Value;
            if (fraudResult.IsSet)
                paymentResponse.FraudResult = fraudResult.Value;
            if (merchantReference.IsSet)
                paymentResponse.MerchantReference = merchantReference.Value;
            if (order.IsSet)
                paymentResponse.Order = order.Value;
            if (paymentMethod.IsSet)
                paymentResponse.PaymentMethod = paymentMethod.Value;
            if (paymentValidations.IsSet)
                paymentResponse.PaymentValidations = paymentValidations.Value;
            if (pspReference.IsSet)
                paymentResponse.PspReference = pspReference.Value;
            if (refusalReason.IsSet)
                paymentResponse.RefusalReason = refusalReason.Value;
            if (refusalReasonCode.IsSet)
                paymentResponse.RefusalReasonCode = refusalReasonCode.Value;
            if (resultCode.IsSet)
                paymentResponse.ResultCode = resultCode.Value;
            if (threeDS2ResponseData.IsSet)
                paymentResponse.ThreeDS2ResponseData = threeDS2ResponseData.Value;
            if (threeDS2Result.IsSet)
                paymentResponse.ThreeDS2Result = threeDS2Result.Value;
            if (threeDSPaymentData.IsSet)
                paymentResponse.ThreeDSPaymentData = threeDSPaymentData.Value;
