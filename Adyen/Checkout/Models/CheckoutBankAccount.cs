...
namespace Adyen.Checkout.Models
{
    ...
    /// CheckoutBankAccount.
    /// </summary>
    [JsonConverter(typeof(CheckoutBankAccountJsonConverter))]
    ...
    public class CheckoutBankAccountJsonConverter : JsonConverter<CheckoutBankAccount>
    {
        ...
        public override CheckoutBankAccount Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (bankAccountNumber.IsSet)
                checkoutBankAccount.BankAccountNumber = bankAccountNumber.Value;
            if (bankCity.IsSet)
                checkoutBankAccount.BankCity = bankCity.Value;
            if (bankLocationId.IsSet)
                checkoutBankAccount.BankLocationId = bankLocationId.Value;
            if (bankName.IsSet)
                checkoutBankAccount.BankName = bankName.Value;
            if (bic.IsSet)
                checkoutBankAccount.Bic = bic.Value;
            if (countryCode.IsSet)
                checkoutBankAccount.CountryCode = countryCode.Value;
            if (iban.IsSet)
                checkoutBankAccount.Iban = iban.Value;
            if (ownerName.IsSet)
                checkoutBankAccount.OwnerName = ownerName.Value;
            if (taxId.IsSet)
                checkoutBankAccount.TaxId = taxId.Value;
            return checkoutBankAccount;
        }
