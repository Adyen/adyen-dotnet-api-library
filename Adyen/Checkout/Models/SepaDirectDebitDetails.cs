...
namespace Adyen.Checkout.Models
{
    ...
    /// </summary>
    [JsonConverter(typeof(SepaDirectDebitDetailsJsonConverter))]
    ...
    public class SepaDirectDebitDetailsJsonConverter : JsonConverter<SepaDirectDebitDetails>
    {
        ...
        public override SepaDirectDebitDetails Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (dueDate.IsSet)
                sepaDirectDebitDetails.DueDate = dueDate.Value;
            if (iban.IsSet)
                sepaDirectDebitDetails.Iban = iban.Value;
            if (ownerName.IsSet)
                sepaDirectDebitDetails.OwnerName = ownerName.Value;
            if (recurringDetailReference.IsSet)
                sepaDirectDebitDetails.RecurringDetailReference = recurringDetailReference.Value;
            if (sdkData.IsSet)
                sepaDirectDebitDetails.SdkData = sdkData.Value;
            if (storedPaymentMethodId.IsSet)
                sepaDirectDebitDetails.StoredPaymentMethodId = storedPaymentMethodId.Value;
            if (transferInstrumentId.IsSet)
                sepaDirectDebitDetails.TransferInstrumentId = transferInstrumentId.Value;
            if (type.IsSet)
                sepaDirectDebitDetails.Type = type.Value;
        ...
        public void WriteProperties(Utf8JsonWriter writer, SepaDirectDebitDetails sepaDirectDebitDetails, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (sepaDirectDebitDetails._IbanOption.IsSet)
                if (sepaDirectDebitDetails.Iban != null)
                    writer.WriteString("iban", sepaDirectDebitDetails.Iban);

            if (sepaDirectDebitDetails._OwnerNameOption.IsSet)
