...
namespace Adyen.Checkout.Models
{
    ...
    /// ForexQuote.
    /// </summary>
    [JsonConverter(typeof(ForexQuoteJsonConverter))]
    ...
    public class ForexQuoteJsonConverter : JsonConverter<ForexQuote>
    {
        ...
        public override ForexQuote Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (accountType.IsSet)
                forexQuote.AccountType = accountType.Value;
            if (baseAmount.IsSet)
                forexQuote.BaseAmount = baseAmount.Value;
            if (buy.IsSet)
                forexQuote.Buy = buy.Value;
            if (interbank.IsSet)
                forexQuote.Interbank = interbank.Value;
            if (reference.IsSet)
                forexQuote.Reference = reference.Value;
            if (sell.IsSet)
                forexQuote.Sell = sell.Value;
            if (signature.IsSet)
                forexQuote.Signature = signature.Value;
            if (source.IsSet)
                forexQuote.Source = source.Value;
            if (type.IsSet)
                forexQuote.Type = type.Value;
            return forexQuote;
        }
