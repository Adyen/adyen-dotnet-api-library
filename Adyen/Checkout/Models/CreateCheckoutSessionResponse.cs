...
namespace Adyen.Checkout.Models
{
    ...
    /// </summary>
    [JsonConverter(typeof(CreateCheckoutSessionResponseJsonConverter))]
    public partial class CreateCheckoutSessionResponse
    {
        ...
        /// <summary>
        /// This is used to track if an optional field is set. If set, <see cref="SessionData"/> will be populated.
        /// </summary>
        [JsonIgnore]
        [global::System.ComponentModel.EditorBrowsable(global::System.ComponentModel.EditorBrowsableState.Never)]
    ...
    public class CreateCheckoutSessionResponseJsonConverter : JsonConverter<CreateCheckoutSessionResponse>
    {
        ...
        public override CreateCheckoutSessionResponse Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (returnUrl.IsSet)
                createCheckoutSessionResponse.ReturnUrl = returnUrl.Value!;
            if (accountInfo.IsSet)
                createCheckoutSessionResponse.AccountInfo = accountInfo.Value;
            if (additionalAmount.IsSet)
                createCheckoutSessionResponse.AdditionalAmount = additionalAmount.Value;
            if (additionalData.IsSet)
                createCheckoutSessionResponse.AdditionalData = additionalData.Value;
            if (allowedPaymentMethods.IsSet)
                createCheckoutSessionResponse.AllowedPaymentMethods = allowedPaymentMethods.Value;
            if (applicationInfo.IsSet)
                createCheckoutSessionResponse.ApplicationInfo = applicationInfo.Value;
            if (authenticationData.IsSet)
                createCheckoutSessionResponse.AuthenticationData = authenticationData.Value;
            if (billingAddress.IsSet)
                createCheckoutSessionResponse.BillingAddress = billingAddress.Value;
            if (blockedPaymentMethods.IsSet)
                createCheckoutSessionResponse.BlockedPaymentMethods = blockedPaymentMethods.Value;
            if (captureDelayHours.IsSet)
                createCheckoutSessionResponse.CaptureDelayHours = captureDelayHours.Value;
            if (channel.IsSet)
                createCheckoutSessionResponse.Channel = channel.Value;
            if (company.IsSet)
                createCheckoutSessionResponse.Company = company.Value;
            if (countryCode.IsSet)
                createCheckoutSessionResponse.CountryCode = countryCode.Value;
            if (dateOfBirth.IsSet)
                createCheckoutSessionResponse.DateOfBirth = dateOfBirth.Value;
            if (deliverAt.IsSet)
                createCheckoutSessionResponse.DeliverAt = deliverAt.Value;
            if (deliveryAddress.IsSet)
                createCheckoutSessionResponse.DeliveryAddress = deliveryAddress.Value;
            if (enableOneClick.IsSet)
                createCheckoutSessionResponse.EnableOneClick = enableOneClick.Value;
            if (enablePayOut.IsSet)
                createCheckoutSessionResponse.EnablePayOut = enablePayOut.Value;
            if (enableRecurring.IsSet)
                createCheckoutSessionResponse.EnableRecurring = enableRecurring.Value;
            if (fundOrigin.IsSet)
                createCheckoutSessionResponse.FundOrigin = fundOrigin.Value;
            if (fundRecipient.IsSet)
                createCheckoutSessionResponse.FundRecipient = fundRecipient.Value;
            if (installmentOptions.IsSet)
                createCheckoutSessionResponse.InstallmentOptions = installmentOptions.Value;
            if (lineItems.IsSet)
                createCheckoutSessionResponse.LineItems = lineItems.Value;
            if (mandate.IsSet)
                createCheckoutSessionResponse.Mandate = mandate.Value;
            if (mcc.IsSet)
                createCheckoutSessionResponse.Mcc = mcc.Value;
            if (merchantOrderReference.IsSet)
                createCheckoutSessionResponse.MerchantOrderReference = merchantOrderReference.Value;
            if (metadata.IsSet)
                createCheckoutSessionResponse.Metadata = metadata.Value;
            if (mode.IsSet)
                createCheckoutSessionResponse.Mode = mode.Value;
            if (mpiData.IsSet)
                createCheckoutSessionResponse.MpiData = mpiData.Value;
            if (platformChargebackLogic.IsSet)
                createCheckoutSessionResponse.PlatformChargebackLogic = platformChargebackLogic.Value;
            if (recurringExpiry.IsSet)
                createCheckoutSessionResponse.RecurringExpiry = recurringExpiry.Value;
            if (recurringFrequency.IsSet)
                createCheckoutSessionResponse.RecurringFrequency = recurringFrequency.Value;
            if (recurringProcessingModel.IsSet)
                createCheckoutSessionResponse.RecurringProcessingModel = recurringProcessingModel.Value;
            if (redirectFromIssuerMethod.IsSet)
                createCheckoutSessionResponse.RedirectFromIssuerMethod = redirectFromIssuerMethod.Value;
            if (redirectToIssuerMethod.IsSet)
                createCheckoutSessionResponse.RedirectToIssuerMethod = redirectToIssuerMethod.Value;
            if (riskData.IsSet)
                createCheckoutSessionResponse.RiskData = riskData.Value;
            if (sessionData.IsSet)
                createCheckoutSessionResponse.SessionData = sessionData.Value;
            if (shopperEmail.IsSet)
                createCheckoutSessionResponse.ShopperEmail = shopperEmail.Value;
            if (shopperIP.IsSet)
                createCheckoutSessionResponse.ShopperIP = shopperIP.Value;
            if (shopperInteraction.IsSet)
                createCheckoutSessionResponse.ShopperInteraction = shopperInteraction.Value;
            if (shopperLocale.IsSet)
                createCheckoutSessionResponse.ShopperLocale = shopperLocale.Value;
            if (shopperName.IsSet)
                createCheckoutSessionResponse.ShopperName = shopperName.Value;
            if (shopperReference.IsSet)
                createCheckoutSessionResponse.ShopperReference = shopperReference.Value;
            if (shopperStatement.IsSet)
                createCheckoutSessionResponse.ShopperStatement = shopperStatement.Value;
            if (showInstallmentAmount.IsSet)
                createCheckoutSessionResponse.ShowInstallmentAmount = showInstallmentAmount.Value;
            if (showRemovePaymentMethodButton.IsSet)
                createCheckoutSessionResponse.ShowRemovePaymentMethodButton = showRemovePaymentMethodButton.Value;
            if (socialSecurityNumber.IsSet)
                createCheckoutSessionResponse.SocialSecurityNumber = socialSecurityNumber.Value;
            if (splitCardFundingSources.IsSet)
                createCheckoutSessionResponse.SplitCardFundingSources = splitCardFundingSources.Value;
            if (splits.IsSet)
                createCheckoutSessionResponse.Splits = splits.Value;
            if (store.IsSet)
                createCheckoutSessionResponse.Store = store.Value;
            if (storeFiltrationMode.IsSet)
                createCheckoutSessionResponse.StoreFiltrationMode = storeFiltrationMode.Value;
            if (storePaymentMethod.IsSet)
                createCheckoutSessionResponse.StorePaymentMethod = storePaymentMethod.Value;
            if (storePaymentMethodMode.IsSet)
                createCheckoutSessionResponse.StorePaymentMethodMode = storePaymentMethodMode.Value;
            if (telephoneNumber.IsSet)
                createCheckoutSessionResponse.TelephoneNumber = telephoneNumber.Value;
            if (themeId.IsSet)
                createCheckoutSessionResponse.ThemeId = themeId.Value;
            if (thirdPartyTokenRedundancyInfo.IsSet)
                createCheckoutSessionResponse.ThirdPartyTokenRedundancyInfo = thirdPartyTokenRedundancyInfo.Value;
            if (threeDS2RequestData.IsSet)
                createCheckoutSessionResponse.ThreeDS2RequestData = threeDS2RequestData.Value;
            if (threeDSAuthenticationOnly.IsSet)
                createCheckoutSessionResponse.ThreeDSAuthenticationOnly = threeDSAuthenticationOnly.Value;
            if (trustedShopper.IsSet)
                createCheckoutSessionResponse.TrustedShopper = trustedShopper.Value;
            if (url.IsSet)
                createCheckoutSessionResponse.Url = url.Value;
        ...
        public void WriteProperties(Utf8JsonWriter writer, CreateCheckoutSessionResponse createCheckoutSessionResponse, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (createCheckoutSessionResponse._ThirdPartyTokenRedundancyInfoOption.IsSet)
            {
                writer.WritePropertyName("thirdPartyTokenRedundancyInfo");
                JsonSerializer.Serialize(writer, createCheckoutSessionResponse.ThirdPartyTokenRedundancyInfo, jsonSerializerOptions);
            }
