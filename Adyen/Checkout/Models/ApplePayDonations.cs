...
namespace Adyen.Checkout.Models
{
    ...
    /// ApplePayDonations.
    /// </summary>
    [JsonConverter(typeof(ApplePayDonationsJsonConverter))]
    ...
    public class ApplePayDonationsJsonConverter : JsonConverter<ApplePayDonations>
    {
        ...
        public override ApplePayDonations Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (fundingSource.IsSet)
                applePayDonations.FundingSource = fundingSource.Value;
            if (recurringDetailReference.IsSet)
                applePayDonations.RecurringDetailReference = recurringDetailReference.Value;
            if (sdkData.IsSet)
                applePayDonations.SdkData = sdkData.Value;
            if (storedPaymentMethodId.IsSet)
                applePayDonations.StoredPaymentMethodId = storedPaymentMethodId.Value;
            if (type.IsSet)
                applePayDonations.Type = type.Value;
            return applePayDonations;
        }
