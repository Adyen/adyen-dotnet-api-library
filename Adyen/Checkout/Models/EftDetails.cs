...
namespace Adyen.Checkout.Models
{
    ...
    /// EftDetails.
    /// </summary>
    [JsonConverter(typeof(EftDetailsJsonConverter))]
    public partial class EftDetails
    {
        /// <summary>
        ...
        public EftDetails()
        {
            _TypeOption = TypeEnum.EftDirectdebitCA;
            OnCreated();
        }
    ...
    public class EftDetailsJsonConverter : JsonConverter<EftDetails>
    {
        ...
        public override EftDetails Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (bankCode.IsSet)
                eftDetails.BankCode = bankCode.Value;
            if (bankLocationId.IsSet)
                eftDetails.BankLocationId = bankLocationId.Value;
            if (checkoutAttemptId.IsSet)
                eftDetails.CheckoutAttemptId = checkoutAttemptId.Value;
            if (ownerName.IsSet)
                eftDetails.OwnerName = ownerName.Value;
            if (recurringDetailReference.IsSet)
                eftDetails.RecurringDetailReference = recurringDetailReference.Value;
            if (sdkData.IsSet)
                eftDetails.SdkData = sdkData.Value;
            if (storedPaymentMethodId.IsSet)
                eftDetails.StoredPaymentMethodId = storedPaymentMethodId.Value;
            if (type.IsSet)
                eftDetails.Type = type.Value;
            return eftDetails;
        }
        ...
        public void WriteProperties(Utf8JsonWriter writer, EftDetails eftDetails, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (eftDetails._SdkDataOption.IsSet)
                if (eftDetails.SdkData != null)
                    writer.WriteString("sdkData", eftDetails.SdkData);
