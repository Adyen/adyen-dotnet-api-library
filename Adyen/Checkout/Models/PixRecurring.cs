...
namespace Adyen.Checkout.Models
{
    ...
    /// PixRecurring.
    /// </summary>
    [JsonConverter(typeof(PixRecurringJsonConverter))]
    ...
    public class PixRecurringJsonConverter : JsonConverter<PixRecurring>
    {
        ...
        public override PixRecurring Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (businessDayOnly.IsSet)
                pixRecurring.BusinessDayOnly = businessDayOnly.Value;
            if (endsAt.IsSet)
                pixRecurring.EndsAt = endsAt.Value;
            if (frequency.IsSet)
                pixRecurring.Frequency = frequency.Value;
            if (minAmount.IsSet)
                pixRecurring.MinAmount = minAmount.Value;
            if (originalPspReference.IsSet)
                pixRecurring.OriginalPspReference = originalPspReference.Value;
            if (recurringAmount.IsSet)
                pixRecurring.RecurringAmount = recurringAmount.Value;
            if (recurringStatement.IsSet)
                pixRecurring.RecurringStatement = recurringStatement.Value;
            if (retryPolicy.IsSet)
                pixRecurring.RetryPolicy = retryPolicy.Value;
            if (startsAt.IsSet)
                pixRecurring.StartsAt = startsAt.Value;
            return pixRecurring;
        }
