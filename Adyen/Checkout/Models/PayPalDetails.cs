...
namespace Adyen.Checkout.Models
{
    ...
    /// PayPalDetails.
    /// </summary>
    [JsonConverter(typeof(PayPalDetailsJsonConverter))]
    ...
    public class PayPalDetailsJsonConverter : JsonConverter<PayPalDetails>
    {
        ...
        public override PayPalDetails Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (checkoutAttemptId.IsSet)
                payPalDetails.CheckoutAttemptId = checkoutAttemptId.Value;
            if (orderID.IsSet)
                payPalDetails.OrderID = orderID.Value;
            if (payeePreferred.IsSet)
                payPalDetails.PayeePreferred = payeePreferred.Value;
            if (payerID.IsSet)
                payPalDetails.PayerID = payerID.Value;
            if (payerSelected.IsSet)
                payPalDetails.PayerSelected = payerSelected.Value;
            if (recurringDetailReference.IsSet)
                payPalDetails.RecurringDetailReference = recurringDetailReference.Value;
            if (sdkData.IsSet)
                payPalDetails.SdkData = sdkData.Value;
