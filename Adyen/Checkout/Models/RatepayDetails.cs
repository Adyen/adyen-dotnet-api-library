...
namespace Adyen.Checkout.Models
{
    ...
    /// RatepayDetails.
    /// </summary>
    [JsonConverter(typeof(RatepayDetailsJsonConverter))]
    ...
    public class RatepayDetailsJsonConverter : JsonConverter<RatepayDetails>
    {
        ...
        public override RatepayDetails Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (billingAddress.IsSet)
                ratepayDetails.BillingAddress = billingAddress.Value;
            if (checkoutAttemptId.IsSet)
                ratepayDetails.CheckoutAttemptId = checkoutAttemptId.Value;
            if (deliveryAddress.IsSet)
                ratepayDetails.DeliveryAddress = deliveryAddress.Value;
            if (personalDetails.IsSet)
                ratepayDetails.PersonalDetails = personalDetails.Value;
            if (recurringDetailReference.IsSet)
                ratepayDetails.RecurringDetailReference = recurringDetailReference.Value;
            if (sdkData.IsSet)
