...
namespace Adyen.Checkout.Models
{
    ...
    public class DonationCampaignJsonConverter : JsonConverter<DonationCampaign>
    {
        ...
        public override DonationCampaign Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (bannerUrl.IsSet)
                donationCampaign.BannerUrl = bannerUrl.Value;
            if (campaignName.IsSet)
                donationCampaign.CampaignName = campaignName.Value;
            if (causeName.IsSet)
                donationCampaign.CauseName = causeName.Value;
            if (donation.IsSet)
                donationCampaign.Donation = donation.Value;
            if (id.IsSet)
                donationCampaign.Id = id.Value;
            if (logoUrl.IsSet)
                donationCampaign.LogoUrl = logoUrl.Value;
            if (nonprofitDescription.IsSet)
                donationCampaign.NonprofitDescription = nonprofitDescription.Value;
            if (nonprofitName.IsSet)
                donationCampaign.NonprofitName = nonprofitName.Value;
            if (nonprofitUrl.IsSet)
                donationCampaign.NonprofitUrl = nonprofitUrl.Value;
            if (termsAndConditionsUrl.IsSet)
                donationCampaign.TermsAndConditionsUrl = termsAndConditionsUrl.Value;
