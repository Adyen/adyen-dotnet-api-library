...
namespace Adyen.Checkout.Models
{
    ...
    /// IdealDonations.
    /// </summary>
    [JsonConverter(typeof(IdealDonationsJsonConverter))]
    ...
    public class IdealDonationsJsonConverter : JsonConverter<IdealDonations>
    {
        ...
        public override IdealDonations Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (issuer.IsSet)
                idealDonations.Issuer = issuer.Value;
            if (recurringDetailReference.IsSet)
                idealDonations.RecurringDetailReference = recurringDetailReference.Value;
            if (sdkData.IsSet)
                idealDonations.SdkData = sdkData.Value;
            if (storedPaymentMethodId.IsSet)
                idealDonations.StoredPaymentMethodId = storedPaymentMethodId.Value;
            if (type.IsSet)
                idealDonations.Type = type.Value;
            return idealDonations;
        }
