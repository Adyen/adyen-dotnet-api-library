...
namespace Adyen.Checkout.Models
{
    ...
    /// CardDonations.
    /// </summary>
    [JsonConverter(typeof(CardDonationsJsonConverter))]
    public partial class CardDonations
    {
        /// <summary>
    ...
    public class CardDonationsJsonConverter : JsonConverter<CardDonations>
    {
        ...
        public override CardDonations Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
                ...
                if (utf8JsonReader.TokenType == JsonTokenType.PropertyName && currentDepth == utf8JsonReader.CurrentDepth - 1)
                {
                    ...
                    switch (jsonPropertyName)
                    {
                        ...
                        case "fundingSource":
                            ...
                            fundingSource = new Option<CardDonations.FundingSourceEnum?>(CardDonations.FundingSourceEnum.FromStringOrDefault(fundingSourceRawValue) ?? (CardDonations.FundingSourceEnum)fundingSourceRawValue);
                            break;
                        case "holderName":
                        ...
                        case "type":
                            ...
                            type = new Option<CardDonations.TypeEnum?>(CardDonations.TypeEnum.FromStringOrDefault(typeRawValue) ?? (CardDonations.TypeEnum)typeRawValue);
                            break;
                        default:
                            break;
                    }
            ...
            if (brand.IsSet)
                cardDonations.Brand = brand.Value;
            if (checkoutAttemptId.IsSet)
                cardDonations.CheckoutAttemptId = checkoutAttemptId.Value;
            if (cupsecureplusSmscode.IsSet)
                cardDonations.CupsecureplusSmscode = cupsecureplusSmscode.Value;
            if (cvc.IsSet)
                cardDonations.Cvc = cvc.Value;
            if (encryptedCard.IsSet)
                cardDonations.EncryptedCard = encryptedCard.Value;
            if (encryptedCardNumber.IsSet)
                cardDonations.EncryptedCardNumber = encryptedCardNumber.Value;
            if (encryptedExpiryMonth.IsSet)
                cardDonations.EncryptedExpiryMonth = encryptedExpiryMonth.Value;
            if (encryptedExpiryYear.IsSet)
                cardDonations.EncryptedExpiryYear = encryptedExpiryYear.Value;
            if (encryptedPassword.IsSet)
                cardDonations.EncryptedPassword = encryptedPassword.Value;
            if (encryptedSecurityCode.IsSet)
                cardDonations.EncryptedSecurityCode = encryptedSecurityCode.Value;
            if (expiryMonth.IsSet)
                cardDonations.ExpiryMonth = expiryMonth.Value;
            if (expiryYear.IsSet)
                cardDonations.ExpiryYear = expiryYear.Value;
            if (fastlaneData.IsSet)
                cardDonations.FastlaneData = fastlaneData.Value;
            if (fundingSource.IsSet)
                cardDonations.FundingSource = fundingSource.Value;
            if (holderName.IsSet)
                cardDonations.HolderName = holderName.Value;
            if (networkPaymentReference.IsSet)
                cardDonations.NetworkPaymentReference = networkPaymentReference.Value;
            if (number.IsSet)
                cardDonations.Number = number.Value;
            if (recurringDetailReference.IsSet)
                cardDonations.RecurringDetailReference = recurringDetailReference.Value;
            if (sdkData.IsSet)
                cardDonations.SdkData = sdkData.Value;
            if (shopperNotificationReference.IsSet)
                cardDonations.ShopperNotificationReference = shopperNotificationReference.Value;
            if (srcCorrelationId.IsSet)
                cardDonations.SrcCorrelationId = srcCorrelationId.Value;
            if (srcDigitalCardId.IsSet)
                cardDonations.SrcDigitalCardId = srcDigitalCardId.Value;
            if (srcScheme.IsSet)
                cardDonations.SrcScheme = srcScheme.Value;
            if (srcTokenReference.IsSet)
                cardDonations.SrcTokenReference = srcTokenReference.Value;
            if (storedPaymentMethodId.IsSet)
                cardDonations.StoredPaymentMethodId = storedPaymentMethodId.Value;
            if (threeDS2SdkVersion.IsSet)
                cardDonations.ThreeDS2SdkVersion = threeDS2SdkVersion.Value;
            if (type.IsSet)
                cardDonations.Type = type.Value;
            return cardDonations;
        }
        ...
        public void WriteProperties(Utf8JsonWriter writer, CardDonations cardDonations, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (cardDonations._EncryptedExpiryMonthOption.IsSet)
                if (cardDonations.EncryptedExpiryMonth != null)
