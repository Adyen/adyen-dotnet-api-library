...
namespace Adyen.BinLookup.Models
{
    ...
    /// CardBin.
    /// </summary>
    [JsonConverter(typeof(CardBinJsonConverter))]
    ...
    public class CardBinJsonConverter : JsonConverter<CardBin>
    {
        ...
        public override CardBin Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (commercial.IsSet)
                cardBin.Commercial = commercial.Value;
            if (fundingSource.IsSet)
                cardBin.FundingSource = fundingSource.Value;
            if (fundsAvailability.IsSet)
                cardBin.FundsAvailability = fundsAvailability.Value;
            if (issuerBin.IsSet)
                cardBin.IssuerBin = issuerBin.Value;
            if (issuingBank.IsSet)
                cardBin.IssuingBank = issuingBank.Value;
            if (issuingCountry.IsSet)
                cardBin.IssuingCountry = issuingCountry.Value;
            if (issuingCurrency.IsSet)
                cardBin.IssuingCurrency = issuingCurrency.Value;
            if (paymentMethod.IsSet)
                cardBin.PaymentMethod = paymentMethod.Value;
            if (payoutEligible.IsSet)
                cardBin.PayoutEligible = payoutEligible.Value;
            if (summary.IsSet)
                cardBin.Summary = summary.Value;
            return cardBin;
        }
