...
namespace Adyen.LegalEntityManagement.Models
{
    ...
    /// Trust.
    /// </summary>
    [JsonConverter(typeof(TrustJsonConverter))]
    ...
    public class TrustJsonConverter : JsonConverter<Trust>
    {
        ...
        public override Trust Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (description.IsSet)
                trust.Description = description.Value;
            if (doingBusinessAs.IsSet)
                trust.DoingBusinessAs = doingBusinessAs.Value;
            if (doingBusinessAsAbsent.IsSet)
                trust.DoingBusinessAsAbsent = doingBusinessAsAbsent.Value;
            if (principalPlaceOfBusiness.IsSet)
                trust.PrincipalPlaceOfBusiness = principalPlaceOfBusiness.Value;
            if (registrationNumber.IsSet)
                trust.RegistrationNumber = registrationNumber.Value;
            if (taxInformation.IsSet)
                trust.TaxInformation = taxInformation.Value;
            if (undefinedBeneficiaryInfo.IsSet)
                trust.UndefinedBeneficiaryInfo = undefinedBeneficiaryInfo.Value;
            if (vatAbsenceReason.IsSet)
                trust.VatAbsenceReason = vatAbsenceReason.Value;
            if (vatNumber.IsSet)
                trust.VatNumber = vatNumber.Value;
            return trust;
        }
