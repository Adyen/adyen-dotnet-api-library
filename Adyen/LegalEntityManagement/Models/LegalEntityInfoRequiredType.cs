...
namespace Adyen.LegalEntityManagement.Models
{
    ...
    /// </summary>
    [JsonConverter(typeof(LegalEntityInfoRequiredTypeJsonConverter))]
    public partial class LegalEntityInfoRequiredType
    {
        /// <summary>
    ...
    public class LegalEntityInfoRequiredTypeJsonConverter : JsonConverter<LegalEntityInfoRequiredType>
    {
        ...
        public override LegalEntityInfoRequiredType Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (entityAssociations.IsSet)
                legalEntityInfoRequiredType.EntityAssociations = entityAssociations.Value;
            if (individual.IsSet)
                legalEntityInfoRequiredType.Individual = individual.Value;
            if (organization.IsSet)
                legalEntityInfoRequiredType.Organization = organization.Value;
            if (reference.IsSet)
                legalEntityInfoRequiredType.Reference = reference.Value;
            if (soleProprietorship.IsSet)
                legalEntityInfoRequiredType.SoleProprietorship = soleProprietorship.Value;
            if (trust.IsSet)
                legalEntityInfoRequiredType.Trust = trust.Value;
            if (unincorporatedPartnership.IsSet)
                legalEntityInfoRequiredType.UnincorporatedPartnership = unincorporatedPartnership.Value;
            if (verificationPlan.IsSet)
                legalEntityInfoRequiredType.VerificationPlan = verificationPlan.Value;
