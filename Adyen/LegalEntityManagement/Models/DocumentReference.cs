    ...
    public class DocumentReferenceJsonConverter : JsonConverter<DocumentReference>
    {
        ...
        public override DocumentReference Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (description.IsSet)
                documentReference.Description = description.Value;
            if (fileName.IsSet)
                documentReference.FileName = fileName.Value;
            if (id.IsSet)
                documentReference.Id = id.Value;
            if (modificationDate.IsSet)
                documentReference.ModificationDate = modificationDate.Value;
            if (pages.IsSet)
                documentReference.Pages = pages.Value;
            if (type.IsSet)
                documentReference.Type = type.Value;
