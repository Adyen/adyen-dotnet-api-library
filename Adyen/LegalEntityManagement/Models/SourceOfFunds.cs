...
namespace Adyen.LegalEntityManagement.Models
{
    ...
    /// SourceOfFunds.
    /// </summary>
    [JsonConverter(typeof(SourceOfFundsJsonConverter))]
    ...
    public class SourceOfFundsJsonConverter : JsonConverter<SourceOfFunds>
    {
        ...
        public override SourceOfFunds Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (assetMonthsHeld.IsSet)
                sourceOfFunds.AssetMonthsHeld = assetMonthsHeld.Value;
            if (cryptocurrencyExchange.IsSet)
                sourceOfFunds.CryptocurrencyExchange = cryptocurrencyExchange.Value;
            if (dateOfFundsReceived.IsSet)
                sourceOfFunds.DateOfFundsReceived = dateOfFundsReceived.Value;
            if (dateOfSourceEvent.IsSet)
                sourceOfFunds.DateOfSourceEvent = dateOfSourceEvent.Value;
            if (description.IsSet)
                sourceOfFunds.Description = description.Value;
            if (financiers.IsSet)
                sourceOfFunds.Financiers = financiers.Value;
            if (originatorLegalEntityId.IsSet)
                sourceOfFunds.OriginatorLegalEntityId = originatorLegalEntityId.Value;
            if (purpose.IsSet)
                sourceOfFunds.Purpose = purpose.Value;
            if (relationship.IsSet)
                sourceOfFunds.Relationship = relationship.Value;
            if (type.IsSet)
                sourceOfFunds.Type = type.Value;
            if (website.IsSet)
                sourceOfFunds.Website = website.Value;
