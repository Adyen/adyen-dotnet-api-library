...
namespace Adyen.LegalEntityManagement.Models
{
    ...
    /// LegalEntityInfo.
    /// </summary>
    [JsonConverter(typeof(LegalEntityInfoJsonConverter))]
    ...
    public class LegalEntityInfoJsonConverter : JsonConverter<LegalEntityInfo>
    {
        ...
        public override LegalEntityInfo Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (entityAssociations.IsSet)
                legalEntityInfo.EntityAssociations = entityAssociations.Value;
            if (individual.IsSet)
                legalEntityInfo.Individual = individual.Value;
            if (organization.IsSet)
                legalEntityInfo.Organization = organization.Value;
            if (reference.IsSet)
                legalEntityInfo.Reference = reference.Value;
            if (soleProprietorship.IsSet)
                legalEntityInfo.SoleProprietorship = soleProprietorship.Value;
            if (trust.IsSet)
                legalEntityInfo.Trust = trust.Value;
            if (type.IsSet)
                legalEntityInfo.Type = type.Value;
            if (unincorporatedPartnership.IsSet)
                legalEntityInfo.UnincorporatedPartnership = unincorporatedPartnership.Value;
            if (verificationPlan.IsSet)
                legalEntityInfo.VerificationPlan = verificationPlan.Value;
        ...
        public void WriteProperties(Utf8JsonWriter writer, LegalEntityInfo legalEntityInfo, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (legalEntityInfo._ReferenceOption.IsSet)
                if (legalEntityInfo.Reference != null)
            ...
            if (legalEntityInfo._VerificationPlanOption.IsSet)
                if (legalEntityInfo.VerificationPlan != null)
