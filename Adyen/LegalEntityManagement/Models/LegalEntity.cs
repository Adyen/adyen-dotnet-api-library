...
namespace Adyen.LegalEntityManagement.Models
{
    ...
    /// LegalEntity.
    /// </summary>
    [JsonConverter(typeof(LegalEntityJsonConverter))]
    ...
    public class LegalEntityJsonConverter : JsonConverter<LegalEntity>
    {
        ...
        public override LegalEntity Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
                ...
                if (utf8JsonReader.TokenType == JsonTokenType.PropertyName && currentDepth == utf8JsonReader.CurrentDepth - 1)
                {
                    ...
                    switch (jsonPropertyName)
                    {
                        ...
                        case "type":
                            ...
                            type = new Option<LegalEntity.TypeEnum?>(LegalEntity.TypeEnum.FromStringOrDefault(typeRawValue) ?? (LegalEntity.TypeEnum)typeRawValue);
                            break;
            ...
            if (documentDetails.IsSet)
                legalEntity.DocumentDetails = documentDetails.Value;
            if (documents.IsSet)
                legalEntity.Documents = documents.Value;
            if (entityAssociations.IsSet)
                legalEntity.EntityAssociations = entityAssociations.Value;
            if (individual.IsSet)
                legalEntity.Individual = individual.Value;
            if (organization.IsSet)
                legalEntity.Organization = organization.Value;
            if (problems.IsSet)
                legalEntity.Problems = problems.Value;
            if (reference.IsSet)
                legalEntity.Reference = reference.Value;
            if (soleProprietorship.IsSet)
                legalEntity.SoleProprietorship = soleProprietorship.Value;
            if (transferInstruments.IsSet)
                legalEntity.TransferInstruments = transferInstruments.Value;
            if (trust.IsSet)
                legalEntity.Trust = trust.Value;
            if (type.IsSet)
                legalEntity.Type = type.Value;
            if (unincorporatedPartnership.IsSet)
                legalEntity.UnincorporatedPartnership = unincorporatedPartnership.Value;
            if (verificationDeadlines.IsSet)
                legalEntity.VerificationDeadlines = verificationDeadlines.Value;
            if (verificationPlan.IsSet)
                legalEntity.VerificationPlan = verificationPlan.Value;
            return legalEntity;
        }
