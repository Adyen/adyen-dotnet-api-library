    ...
    public class UnincorporatedPartnershipJsonConverter : JsonConverter<UnincorporatedPartnership>
    {
        ...
        public override UnincorporatedPartnership Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (description.IsSet)
                unincorporatedPartnership.Description = description.Value;
            if (doingBusinessAs.IsSet)
                unincorporatedPartnership.DoingBusinessAs = doingBusinessAs.Value;
            if (doingBusinessAsAbsent.IsSet)
                unincorporatedPartnership.DoingBusinessAsAbsent = doingBusinessAsAbsent.Value;
            if (principalPlaceOfBusiness.IsSet)
                unincorporatedPartnership.PrincipalPlaceOfBusiness = principalPlaceOfBusiness.Value;
            if (registrationNumber.IsSet)
                unincorporatedPartnership.RegistrationNumber = registrationNumber.Value;
            if (taxInformation.IsSet)
                unincorporatedPartnership.TaxInformation = taxInformation.Value;
            if (type.IsSet)
                unincorporatedPartnership.Type = type.Value;
            if (vatAbsenceReason.IsSet)
                unincorporatedPartnership.VatAbsenceReason = vatAbsenceReason.Value;
            if (vatNumber.IsSet)
                unincorporatedPartnership.VatNumber = vatNumber.Value;
