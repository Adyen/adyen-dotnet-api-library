...
namespace Adyen.LegalEntityManagement.Models
{
    ...
    public class IndividualJsonConverter : JsonConverter<Individual>
    {
        ...
        public override Individual Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (identificationData.IsSet)
                individual.IdentificationData = identificationData.Value;
            if (nationality.IsSet)
                individual.Nationality = nationality.Value;
            if (phone.IsSet)
                individual.Phone = phone.Value;
            if (support.IsSet)
                individual.Support = support.Value;
            if (taxInformation.IsSet)
                individual.TaxInformation = taxInformation.Value;
            if (webData.IsSet)
                individual.WebData = webData.Value;
            return individual;
        }
