      ...
      const string jsonPayload = @"
{
  ...
  ""type"": ""balancePlatform.accountHolder.updated""
      ...
      Assert.AreEqual("Add 'organization.entityAssociations' of type 'uboThroughControl' to legal entity",
        accountHolderUpdatedWebhook.Data.AccountHolder.Capabilities["sendToTransferInstrument"].Problems[3].VerificationErrors[0].RemediatingActions[0].Message);
      ...
      const string jsonPayload = @"
{
  ...
  'type': 'balancePlatform.negativeBalanceCompensationWarning.scheduled'
    ...
    [TestMethod]
    public void Test_BalanceNotificationRequestWebhook()
    {
      // Arrange
      string json = @"{
        ""data"": {
      ...
      // Act
      _balancePlatformWebhookHandler.GetBalanceNotificationRequest(json, out BalanceNotificationRequest target);

      // Assert
      Assert.IsNotNull(target);
      Assert.AreEqual("YOUR_BALANCE_PLATFORM", target.Data.BalancePlatform);
      Assert.AreEqual("BWHS00000000000000000000000001", target.Data.SettingIds[0]);
      Assert.AreEqual(DateTime.Parse("2025-01-19T13:37:38+02:00"), target.Data.CreationDate);
      Assert.AreEqual("USD", target.Data.Currency);
      Assert.AreEqual(499900, target.Data.Balances.Available);
      Assert.AreEqual(350000, target.Data.Balances.Pending);
      Assert.AreEqual(120000, target.Data.Balances.Reserved);
      Assert.AreEqual(470000, target.Data.Balances.Balance);
      Assert.AreEqual("test", target.Environment);
      Assert.AreEqual(BalanceNotificationRequest.TypeEnum.BalancePlatformBalanceAccountBalanceUpdated, target.Type);
    }
  }
}